...
cloudswap_contract = get_contract(cloudswap_addr, 'cloud_dapp/swap_abi.json')
...
def swap_token2eth(key: str, token_addr: str):
    ...
    token_balance = token_approve(key, token_addr, cloudswap_addr)
...
def swap_token2token(key: str, token_ori_addr: str, token_tar_addr: str):
    account: LocalAccount = Account.from_key(key)
    addr = account.address
    token_balance = token_approve(key, token_ori_addr, cloudswap_addr)
