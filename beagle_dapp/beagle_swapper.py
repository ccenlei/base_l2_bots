...
beagleswap_contract = get_contract(beagleswap_addr, 'beagle_dapp/swap_abi.json', w3)
...
def swap_eth2token(key: str, token_addr: str, amount=0.0121):
    ...
    sign_tx(tx_dict, account, w3, 'beagle swap eth for token')

def swap_token2eth(key: str, token_addr: str):
    account: LocalAccount = Account.from_key(key)
    addr = account.address
    token_balance = token_approve(key, token_addr, beagleswap_addr, w3)
    ...
    sign_tx(tx_dict, account, w3, 'beagle swap token for eth')

def swap_token2token(key: str, token_ori_addr: str, token_tar_addr: str):
    account: LocalAccount = Account.from_key(key)
    addr = account.address
    token_balance = token_approve(key, token_ori_addr, beagleswap_addr, w3)
    ...
    sign_tx(tx_dict, account, w3, 'beagle swap token for token')
